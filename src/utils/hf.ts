...
// -----------------------------
// ASR helpers (Whisper .bin or .gguf)
// -----------------------------

/**
 * Filters Whisper / ASR model files from model siblings.
 * Includes both .bin (ggml) and .gguf formats so available GGUF files are displayed.
 * Excludes sharded GGUF files (e.g. *-00001-of-00002.gguf).
 */
export function filterValidAsrBinFiles(siblings: any[]): any[] {
  return (
    siblings?.filter(sibling => {
      const filename = (sibling.rfilename || sibling.path || '').toLowerCase();
...
export function normalizeAsrModelSiblings(
  modelId: string,
  siblings: any[],
): ModelFile[] {
  const filteredSiblings = filterValidAsrBinFiles(siblings).map(sibling => ({
    ...sibling,
    rfilename: sibling.rfilename || sibling.path || '',
  }));
  return addModelFileDownloadUrls(modelId, filteredSiblings);
}

export function processHFAsrSearchResults(
  models: HuggingFaceModel[],
): HuggingFaceModel[] {
  return models.map(model => ({
    ...model,
    url: urls.modelWebPage(model.id),
    siblings: normalizeAsrModelSiblings(model.id, model.siblings || []),
  }));
}
