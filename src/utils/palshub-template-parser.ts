...
export function parsePalsHubTemplate(template: string): ParsedMustacheTemplate {
  ...
  // Convert schema to Sikia format
  const parameterSchema = schema ? convertJsonSchemaToSikia(schema) : [];
...
/**
 * Converts JSON schema to Sikia ParameterDefinition format
 */
function convertJsonSchemaToSikia(
  ...
  for (const [key, definition] of Object.entries(schema)) {
    ...
    // Map schema types to Sikia types
