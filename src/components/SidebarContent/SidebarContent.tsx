...
import {Divider, Drawer, Text, Badge} from 'react-native-paper';
...
import {chatSessionStore, syncStore, SessionMetaData} from '../../store';
import {formResponseStore} from '../../store/FormResponseStore';
import {clearProjectsCache} from '../../services/mobileApi';
...
import {ROUTES, FEATURE_FLAGS} from '../../utils/navigationConstants';
      ...
      () => (
        <View>
          <Drawer.Section showDivider={false}>
              ...
              right={() =>
                syncStore.pendingCount > 0 ? (
                  <View style={{marginRight: 8}}>
                    <Badge size={18}>{syncStore.pendingCount}</Badge>
                  </View>
                ) : null
              }
              onPress={() =>
                props.navigation.navigate(ROUTES.SESSION as never, {
                  screen: 'Projects',
                } as never)
              }
              style={styles.menuDrawerItem}
              testID="drawer-item-session"
            />
            {FEATURE_FLAGS.ENABLE_PALS && (
              <Drawer.Item
            ...
            />
            {FEATURE_FLAGS.ENABLE_BENCHMARK && (
              <Drawer.Item
            ...
            <Drawer.Item
              ...
              style={styles.menuDrawerItem}
            />
            {/* Only show Dev Tools in debug mode (same on iOS and Android). Run Android in debug, e.g. npx react-native run-android, to see it. */}
            {isDebugMode && (
              <Drawer.Item
                label="Dev Tools"
                icon={() => (
            ...
            <Drawer.Item
              ...
              onPress={async () => {
                try {
                  formResponseStore.clearAll();
                  clearProjectsCache();
                  await sikiaAuthService.signOut();
