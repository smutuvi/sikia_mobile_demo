...
export const ChatView = observer(
  ({
    ...
    // Voice input (Offline = on-device ASR, Online = OpenAI Whisper): insert transcript into chat input
    const onVoiceResult = React.useCallback((text: string) => {
      setInputText(prev => (prev ? `${prev} ${text}` : text));
    }, []);
    const [chatSttMode, setChatSttMode] = React.useState<SttMode>('online');
    const activeAsrModel = asrModelStore.activeModel;
    const hasOfflineAsrModel = Boolean(activeAsrModel?.isDownloaded);
    const isOnlineSttAvailable = isOnlineSttConfigured();
    const isOfflineSttAvailable = hasOfflineAsrModel;
    const {
    ...
    React.useEffect(() => {
      if (isVoiceRecording) setHasVoiceRecordingStarted(true);
    }, [isVoiceRecording]);
    React.useEffect(() => {
      if (!isVoiceTranscribing && !isVoiceRecording) {
        setHasVoiceRecordingStarted(false);
      }
    }, [isVoiceTranscribing, isVoiceRecording]);
    const voiceStatusLabel =
      isVoiceTranscribing && !isVoiceRecording
        ? hasVoiceRecordingStarted
          ? onlineStep || 'Transcribing…'
          : 'Starting…'
        : isVoiceRecording
          ? 'Listening…'
          : '';
    const showVoiceInput =
      (isOnlineSttAvailable || isOfflineSttAvailable) &&
      !(activePal && hasVideoCapability(activePal));
    const voiceModeConfigured =
      chatSttMode === 'online' ? isOnlineSttAvailable : isOfflineSttAvailable;
            ...
            <Reanimated.View
              ...
              {showVoiceInput && (
                <View style={{flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingBottom: 8}}>
                  <SegmentedButtons
                    value={chatSttMode}
                    onValueChange={(v) => setChatSttMode(v as SttMode)}
                    buttons={[
                    ...
                    style={{flex: 0}}
                  />
                  {chatSttMode === 'offline' && !isOfflineSttAvailable && (
                    <Text variant="bodySmall" style={{color: theme.colors.error, marginLeft: 8}} numberOfLines={1}>
                      No ASR model
                    </Text>
                  )}
                </View>
              ...
              <ChatInput
                {...{
                  ...
                  isVoiceTranscribing,
                  voiceError,
                  onStartVoice: voiceModeConfigured
                    ? () => {
                        setInputText('');
                        startVoice();
                      }
                    : undefined,
                  onStopVoice: voiceModeConfigured ? stopVoice : undefined,
                  voiceStatusLabel,
                  textInputProps: {
                    ...textInputProps,
