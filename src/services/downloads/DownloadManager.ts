...
export class DownloadManager {
  ...
  private callbacksList: DownloadEventCallbacks[] = [];
  ...
  private setupAndroidEventListener() {
    if (NativeDownloadModule) {
      ...
      this.eventEmitter.addListener('onDownloadProgress', event => {
        ...
        this.emit('onProgress', job.model.id, progress);
      });
      ...
      this.eventEmitter.addListener('onDownloadComplete', event => {
        ...
        if (job) {
          ...
          this.emit('onComplete', job.model.id);
      ...
      this.eventEmitter.addListener('onDownloadFailed', event => {
        ...
        if (job) {
          ...
          job.state.isDownloading = false;
          // Ensure callback is called before removing the job
          this.emit('onError', job.model.id, new Error(event.error));
          this.downloadJobs.delete(job.model.id);
  ...
  private emit<K extends keyof DownloadEventCallbacks>(
    event: K,
    ...args: Parameters<NonNullable<DownloadEventCallbacks[K]>>
  ) {
    for (const cb of this.callbacksList) {
      const fn = cb[event] as any;
      try {
        fn?.(...args);
      } catch (err) {
        console.warn(`${TAG}: Callback error for ${String(event)}:`, err);
      }
    }
  }
  ...
  setCallbacks(callbacks: DownloadEventCallbacks) {
    ...
    // Backwards compatible: replace all listeners with this one.
    this.callbacksList = [callbacks];
  }

  /**
   * Adds a callbacks listener without overwriting existing ones.
   * Returns an unsubscribe function.
   */
  addCallbacks(callbacks: DownloadEventCallbacks): () => void {
    this.callbacksList = [...this.callbacksList, callbacks];
    return () => {
      this.callbacksList = this.callbacksList.filter(cb => cb !== callbacks);
    };
  }
    ...
    try {
      ...
      const downloadResult = RNFS.downloadFile({
        ...
        begin: res => {
          ...
          this.emit('onProgress', model.id, progress);
        },
        progress: res => {
   ...
   */
  syncWithActiveDownloads = async (models: DownloadableItem[]): Promise<void> => {
