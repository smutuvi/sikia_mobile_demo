...
class ModelStore {
  ...
  private async initializeThreadCount() {
    try {
      ...
      // Bias toward fast interactive use: avoid oversubscribing all cores
      const interactiveThreads = Math.min(Math.max(2, threads), Math.max(4, Math.floor(cores * 0.75)));
      runInAction(() => {
        this.contextInitParams = {
          ...this.contextInitParams,
          n_threads: interactiveThreads,
        };
      });
    } catch (error) {
   ...
   * See: ios/Sikia/AppIntents/PalDataProvider.swift - parseModelPath() method
  ...
  /**
   * Run a single text-only completion (e.g. for Session follow-up generation).
   * Uses the currently loaded model. No streaming; returns full text.
   * @param options.forSessionFollowUp - When true, uses a low-latency preset: lower n_predict, thinking disabled.
   * @returns Promise with the generated text, or empty string if no context or error.
   */
  runTextCompletion = async (
    systemPrompt: string,
    userPrompt: string,
    options?: { forSessionFollowUp?: boolean },
  ): Promise<string> => {
    if (!this.context) {
      if (__DEV__) console.warn('[ModelStore] runTextCompletion: no context loaded');
      return '';
    }
    try {
      ...
      let baseParams: CompletionParams;
      if (options?.forSessionFollowUp) {
        // Isolated minimal params for follow-up: bypass global chat settings entirely.
        // Low temperature for deterministic short answers; no repeat penalty overhead;
        // tight n_predict so generation stops as soon as the 1-2 sentence answer is done.
        baseParams = {
          messages,
          stop: stopWords,
          n_predict: 50,
      ...
      const cleanCompletionParams = toApiCompletionParams(baseParams);
      const completionPromise = this.context.completion(
        cleanCompletionParams,
        () => {},
      );
      ...
      this.clearCompletionPromise();
      if (__DEV__) console.warn('[ModelStore] runTextCompletion error:', error);
      return '';
    }
  };
